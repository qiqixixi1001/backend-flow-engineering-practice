# Backend Flow Engineering 25: UPF and CPF: Why Power Intent Must Be Explicitly Understood by the Backend Tool

> Author: Darren H. Chen  
> Direction: Backend Flow / Physical Implementation / EDA Tool Engineering / Tcl-Based Flow Engineering  
> demo: **LAY-BE-25_upf_cpf**  
> Tags: Backend Flow, UPF, CPF, Power Intent, Power State, Low Power

Power intent is design semantics. It cannot be safely inferred from RTL text alone and must be represented as a tool-understood model.
...
- `strategy mapper`
- `implementation refinement layer`
- `consistency checker`
...
- `separate logical function from power architecture`
- `check domain scope and element membership`
- `validate strategy coverage for boundary signals`
- `treat power states as legal operating modes`
- `report how intent is refined through the flow`
...
- `supply set mapping`
- `strategy coverage`
- `power-state table or equivalent mode data`
- `special-cell mapping readiness`
...
- `power-intent files are treated as optional side files`
- `domain scope does not match hierarchy`
- `strategies exist but apply to empty object sets`
- `power-state combinations are not checked`
...
```text
docs/articles/25_upf_cpf.md
examples/LAY-BE-25_upf_cpf/
examples/LAY-BE-25_upf_cpf/README.md
examples/LAY-BE-25_upf_cpf/reports/
```
...
```text
read power intent early; query domain objects; align HDL, libraries, and physical supplies
```
...
> Can the backend database answer power-domain questions as first-class design questions?
