# Backend Flow Engineering 18: Clock Tree Concept: Why Clock Nets Are Not Ordinary Nets
...
> Tags: Backend Flow, Clock Tree, Latency, Skew, Clock Network, CTS

A clock network distributes time, not just electrical connectivity; it therefore has its own architecture, constraints, and failure modes.
...
- `clock constraint model`
- `sink collection builder`
- `topology planner`
- `buffer selection model`
- `route rule model`
...
- `define clock intent before building topology`
- `group sinks by timing relationship`
- `treat latency and skew as separate quantities`
- `reserve routing resources for clock nets`
- `review clock power as part of the architecture`
...
- `clock source and sink count`
- `skew group membership`
...
- `clock nets are routed as ordinary signal nets`
- `skew groups are built from incomplete sink sets`
- `clock gating cells are ignored in clock planning`
- `clock rules are selected after CTS rather than before it`
...
```text
docs/articles/18_clock_tree_concept.md
examples/LAY-BE-18_clock_tree_concept/
examples/LAY-BE-18_clock_tree_concept/README.md
examples/LAY-BE-18_clock_tree_concept/reports/
```
...
```text
treat clock separately from data nets; define balancing intent; preserve clock quality during optimization
```
...
> Does the clock plan support timing closure instead of only connecting sinks?
