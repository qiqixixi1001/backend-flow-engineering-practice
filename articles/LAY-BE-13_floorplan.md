# Backend Flow Engineering 13: Floorplan: Why Backend Implementation Starts From Boundaries, Rows, Sites, and Power Structure
...
> Tags: Backend Flow, Floorplan, Die, Core, Row, Site, Macro, Power Plan

Floorplan is the spatial contract that defines what later placement, clock, routing, and signoff stages are allowed to solve.
...
- `define the coordinate system before creating physical objects`
- `check die/core/core-margin consistency`
- `treat rows as legal placement grids, not decoration`
- `reserve routing channels before dense placement`
- `turn every floorplan assumption into a report`
...
- `row count, orientation, and site compatibility`
- `macro keepout and channel spacing`
- `blockage coverage`
- `power ring and stripe keepout impact`
...
- `utilization is selected before macro/channel feasibility is checked`
- `rows overlap macros or blockages`
- `power structures are added after the placement search space is frozen`
- `routing resource is ignored until detail route`
...
```text
docs/articles/13_floorplan.md
examples/LAY-BE-13_floorplan/
examples/LAY-BE-13_floorplan/README.md
examples/LAY-BE-13_floorplan/reports/
```
...
```text
define physical search space first; keep row legality visible; reserve routing/power resources early
```
...
> Does the floorplan create a solvable physical problem for placement and routing?
...
## Additional Note: Floorplan Converts Abstract Logic into Physical Capacity

Floorplan is the first point where abstract logic becomes a capacity problem. The design no longer consists only of gates and connections; it now has a boundary, physical rows, macro obstacles, routing channels, power resources, and IO access. A weak floorplan can make a logically correct design physically unsolvable. The engineer should therefore read a floorplan report as a statement about capacity: available placement area, blocked regions, macro channels, pin distribution, and routing resources.
