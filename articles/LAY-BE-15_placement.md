# Backend Flow Engineering 15: Placement: Why Cell Placement Is a Multi-Objective Tradeoff Among Legality, Timing, Congestion, Power, and Repairability
...
> Tags: Backend Flow, Placement, Legalization, Congestion, Timing, Optimization

Placement is not merely putting cells into rows; it is the first large-scale physical optimization step that turns a logical graph into spatial structure.
...
- `check feasibility before optimization quality`
- `separate global placement from legalization`
- `review timing and congestion together`
- `keep a before/after report for every placement run`
- `avoid treating wirelength as the only objective`
...
- `congestion estimate`
- `timing trend`
- `displacement after legalization`
...
- `legal placement is confused with good placement`
- `cell density is fixed while congestion is blamed on routing`
- `placement is performed before floorplan feasibility is checked`
- `timing degradation is discovered only after routing`
...
```text
docs/articles/15_placement.md
examples/LAY-BE-15_placement/
examples/LAY-BE-15_placement/README.md
examples/LAY-BE-15_placement/reports/
```
...
```text
optimize in phases; separate spreading from legality; review density and timing together
```
...
> Does the placement improve one metric by silently damaging another?
...
## Additional Note: Placement Quality Must Be Read as a Multi-Objective Result

A placement that improves timing but creates routing congestion is not healthy. A placement that spreads cells evenly but destroys critical path locality is not healthy either. Placement quality must be interpreted across density, wirelength, timing, routability, legality, and power-related implications. The most important skill is not selecting one metric, but understanding which physical trade-off caused each metric to move.
