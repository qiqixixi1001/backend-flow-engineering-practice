# Backend Flow Engineering 10: From Import to Link: Why the Link Stage Determines Whether the Tool Understands the Design
...
> Tags: Backend Flow, EDA, Design Link, Current Design Context, Library Binding, Netlist Semantics

Import creates objects, but the link stage decides whether those objects form a meaningful design. A netlist instance must bind to a library cell. A top module must become the active design context. Timing and physical views must attach to the same object identities. Without this binding, the tool may have many objects loaded but still not understand the design.

This distinction is critical in backend engineering. A design can be imported and still not be ready for placement, timing, or routing. The link stage is the moment when source files become an analyzable design database.
...
The concept is binding. Import creates candidate objects. Linking resolves references. A module instance points to a cell definition. A cell definition points to a timing and physical library view. A top-level design becomes the root context for object queries and reports.

This is also where name mismatches become visible. If the netlist uses a cell name that is not available in the library, the design cannot be fully resolved. If the top module name is wrong, the tool may load files but fail to establish the active design context. If library views are incomplete, later analysis will be partial.
...
The architecture of this stage includes imported object pools, library binding, top context selection, unresolved reference reporting, and state verification. The imported object pools contain data from previous format readers. Library binding attaches design instances to library definitions. Top context selection defines the root. Reports show unresolved references and current context.

A mature flow makes the link stage visible. It does not hide it inside a long script. It writes a dedicated report that says which top was requested, whether the link stage succeeded, what libraries were visible, and whether unresolved objects remain.
...
The data model changes from disconnected pools to a rooted graph. Before link, objects may exist without complete relationships. After link, the design should have a root, hierarchy, instances, nets, ports, pins, and library bindings.

This rooted graph is the foundation for later stages. Placement queries cells in this graph. Timing queries paths in this graph. Routing connects physical pins from this graph. Physical verification handoff exports layout derived from this graph.
...
- Requested top-level design is recorded.
- Link result is written to a report.
- Loaded libraries are listed.
- Unresolved modules or cells are reported.
- Basic object queries return meaningful results after link.
...
- Assuming import success means the design is linked.
- Using a top name that does not match the netlist.
- Loading libraries but not checking whether instances bind to them.
- Ignoring unresolved references because reports are too long.
- Running object queries before the active design context is established.
...
```text
import first, bind second, verify third; classify unresolved references; make link status a gate
```
...
> Does the tool merely store files, or does it understand the top design?
