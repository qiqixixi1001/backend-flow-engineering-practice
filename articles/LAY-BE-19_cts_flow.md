# Backend Flow Engineering 19: CTS Flow: What Clock Tree Synthesis Really Synthesizes
...
> Tags: Backend Flow, CTS, Clock Tree, Skew Group, Clock Route Rule, Post-CTS

CTS synthesizes a controlled timing distribution network under physical, electrical, and scenario constraints; it is not simply inserting buffers.
...
- `pre-CTS checker`
- `clock topology engine`
- `buffer library selector`
- `route rule resolver`
- `post-CTS timing updater`
- `clock report analyzer`
...
- `perform pre-CTS sanity checks`
- `verify sink completeness`
- `choose buffer classes by electrical need`
- `compare pre-CTS and post-CTS timing explicitly`
- `treat post-CTS optimization as a separate stage`
...
- `sink count per skew group`
- `buffer count and level count`
- `latency distribution`
- `skew histogram`
- `post-CTS setup/hold trend`
- `clock DRC status`
...
- `CTS is launched before placement is stable`
- `clock sinks are missing because constraints are incomplete`
- `hold degradation is not tracked after clock insertion`
- `clock route rules are inconsistent with routing resources`
...
```text
docs/articles/19_cts_flow.md
examples/LAY-BE-19_cts_flow/
examples/LAY-BE-19_cts_flow/README.md
examples/LAY-BE-19_cts_flow/reports/
```
...
```text
prepare sinks and constraints; synthesize topology; verify skew/latency; repair data timing after clock insertion
```
...
> Can the CTS result be reviewed as a designed network, not just a generated net?
