...
```text
connect logical power intent to physical supply structures; check domain crossings; preserve always-on reachability
```
...
> Does the physical implementation realize the intended power architecture safely?
...
## Additional Note: Low Power Turns Physical Implementation into Power-State Implementation

In a low-power design, backend implementation is not only about placing and routing logic. It must preserve behavior across power states. Isolation, retention, always-on paths, level shifting, and power switch structures create physical requirements that cannot be inferred from connectivity alone. The backend database must therefore represent power intent as implementation-relevant state, not as a late annotation.
