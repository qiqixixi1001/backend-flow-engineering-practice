# 11. Why Cell, Net, Pin, and Port Are the Basic Units of Backend Flow Engineering
...
> Tags: Backend Flow, EDA, Design Object Model, Cell, Net, Pin, Port, Property Query

After import and link, backend engineering becomes object engineering. The basic units are no longer lines in a file. They are cells, nets, pins, ports, properties, collections, and relationships. Every placement, timing, routing, ECO, and signoff handoff decision ultimately manipulates or queries these objects.

This is why the design object model is a core topic. Engineers who understand the object model can write safer scripts, debug faster, and build better reports. Engineers who only copy commands often struggle when names, hierarchy, collections, or contexts behave differently from expectation.
...
The concept is typed design representation. A cell is not the same as a pin. A pin is not the same as a port. A net is not the same as a route shape. A property is not the same as the object that owns it. The EDA tool maintains these distinctions because each object type has different relationships and legal operations.

The object model also explains why many commands return collections rather than plain strings. A collection may contain zero, one, or many typed objects. It may be filtered, iterated, reported, or converted to names. Treating it as ordinary text can lead to subtle script bugs.
...
The architecture of object access includes object query, collection creation, filtering, property lookup, report generation, and optional selection. Object query finds objects by type and pattern. Collection creation returns tool-managed handles. Filtering reduces the collection by property or relationship. Property lookup exposes metadata. Reports convert database state into text evidence.

This architecture should be reflected in scripts. A script should first query objects, then check collection size, then filter, then inspect properties, then report. Skipping these steps makes scripts fragile.
...
The data model is a graph. Ports sit at module boundaries. Pins belong to instances. Nets connect pins and ports. Cells represent instances of library cells or hierarchical modules. Properties describe object state. Timing paths, placement rows, route shapes, and violations attach to this graph as later views.

A simplified relation is: port connects top-level interface to nets; nets connect to pins; pins belong to cells; cells reference library cells; library cells provide timing and physical views. This relation is the backbone of backend analysis.
...
- Cell, net, pin, and port queries are executed after link.
- Object counts are written to a report.
- Properties are queried on specific objects, not on wildcards without context.
- Empty collections are reported explicitly.
- The report explains which object type each query targets.
...
- Confusing top-level ports with instance pins.
- Using names when the tool expects object collections.
- Running property queries on the wrong object type.
- Assuming an empty collection is always an error.
- Using graphical selection concepts in a batch flow without checking command support.
...
```text
query objects before editing them; distinguish names from objects; use properties as reviewable state
```
...
> Can every script operation be explained as reading or changing design objects?
