# Backend Flow Engineering 21: DRC, Antenna, and Metal Fill: Why Route Completion Is Not Manufacturing Closure
...
> Tags: Backend Flow, DRC, Antenna, Metal Fill, Manufacturing Closure

A routed design is electrically connected, but it is not necessarily manufacturable; DRC, antenna checking, and metal fill close different manufacturing-related gaps.
...
- `in-design rule checker`
- `antenna checker`
- `fill insertion engine`
- `density analyzer`
- `timing re-evaluator`
- `PV handoff layer`
...
- `classify violations by root cause`
- `fix antenna before final extraction`
- `treat fill as a timing-impacting layout change`
- `compare in-design checks with signoff results`
- `archive rule categories and fix actions`
...
- `DRC by category`
- `antenna violations by net`
- `fill density windows`
- `timing delta after fill`
- `route integrity after fixes`
...
- `DRC count is reduced without understanding rule class`
- `fill is inserted after final timing without re-evaluation`
- `antenna repair creates new congestion`
- `local rule clean is mistaken for signoff clean`
...
```text
docs/articles/21_drc_antenna_fill.md
examples/LAY-BE-21_drc_antenna_fill/
examples/LAY-BE-21_drc_antenna_fill/README.md
examples/LAY-BE-21_drc_antenna_fill/reports/
```
...
```text
classify violations; distinguish local repair from architecture problems; verify fill side effects
```
...
> Can signoff checks be traced back to actionable physical causes?
